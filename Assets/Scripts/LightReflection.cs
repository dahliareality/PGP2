...
// updated 29/10/2015 12:00 for visible aiming
// --------------------------
public class LightReflection : MonoBehaviour {
    ...
    public void drawLaser(Vector3 startPoint, int n)
    {
        ...
        for (int i = 0; i < n; i++) //goes through itself for the ammount of bounces
        {
            ...
            if (Physics.Raycast(startPoint, rayDir, out hit, 10000)) //checks for collision with any object, increased range again
            {
                ...
                if (distance < 26 && hit.collider.gameObject.tag == "Mirror") //added distance constraint
                {
                   n += 1; //increases the value of rayJumps
                   //Debug.Log(n);
                   //hit.collider.gameObject.tag = "Tagged";
                }
                else if (distance < 26 && hit.collider.gameObject.tag == "Light Trigger Object") //added distance constraint
                {
                ...
                //adds a LineRenderer component between start position and the hit point.
